...
def point_to_region_id(cx: float, cy: float, w: int, h: int, num_rows: int = 5, num_cols: int = 2) -> int:
    """
    5x2 分区：
    s1 s2
    s3 s4
    ...
    s9 s10
    """
    col = min(int(cx / max(w, 1) * num_cols), num_cols - 1)
    row = min(int(cy / max(h, 1) * num_rows), num_rows - 1)
    return row * num_cols + col + 1


def bbox_to_region_id(bbox: Tuple[int, int, int, int], w: int, h: int, num_rows: int = 5, num_cols: int = 2) -> int:
    cx, cy = bbox_center(bbox)
    return point_to_region_id(cx, cy, w, h, num_rows=num_rows, num_cols=num_cols)


def region_id_to_bbox(region_id: int, w: int, h: int, num_rows: int = 5, num_cols: int = 2) -> Tuple[int, int, int, int]:
    idx = region_id - 1
    row = idx // num_cols
    col = idx % num_cols
...
def extract_region_from_text(text: str, max_region: int = 10) -> Optional[int]:
    m = re.search(r"region\s*[:：]\s*s(\d+)", text, flags=re.I)
    if not m:
        return None
    rid = int(m.group(1))
    if 1 <= rid <= max_region:
...
# =====================================
# Stage-1 5x2 detector prompts/answers
# =====================================

def build_det_prompt(seq: Optional[str], num_rows: int = 5, num_cols: int = 2) -> str:
    seq_part = f"，序列为{seq}" if seq else ""
    max_region = num_rows * num_cols
    region_list = ",".join([f"s{i}" for i in range(1, max_region + 1)])
    return (
        f"现在你是一个骨科专家，这是一幅脊椎的磁共振图像{seq_part}。"
        f"请观察整张图像，判断图中最主要病灶位于哪个区域。"
        f"整张图像被划分为{num_rows}x{num_cols}个区域，"
        f"区域编号为：{region_list}。"
        f"请严格按照以下格式回答：region:s?"
    )


def build_det_answer(bbox_px: Tuple[int, int, int, int], w: int, h: int, num_rows: int = 5, num_cols: int = 2) -> str:
    region_id = bbox_to_region_id(bbox_px, w, h, num_rows=num_rows, num_cols=num_cols)
    return f"region:s{region_id}"
...
class TwoStageDatasetBuilder:
    def __init__(self, processor, task: str, max_length: int, crop_expand_ratio: float = 0.1, num_rows: int = 5, num_cols: int = 2):
    ...
    def _build_messages_and_answer(self, example: Dict):
        ...
        if self.task == "det":
            prompt = build_det_prompt(seq, num_rows=self.num_rows, num_cols=self.num_cols)
            answer = build_det_answer(bbox, width, height, num_rows=self.num_rows, num_cols=self.num_cols)
...
def predict_region(
    ...
    prompt = build_det_prompt(seq, num_rows=num_rows, num_cols=num_cols)
    ...
    region_id = extract_region_from_text(text, max_region=num_rows * num_cols)
...
def evaluate_detector(model, processor, records: List[Record], log_image_num: int = 20, num_rows: int = 5, num_cols: int = 2):
    total = 0
    region_acc = 0
    iou03 = 0
    iou05 = 0
    ...
    for rec in pbar:
        ...
        gt_region = bbox_to_region_id(rec.bbox_px, rec.width, rec.height, num_rows=num_rows, num_cols=num_cols)

        iou = 0.0
        if pred_region is not None:
            region_acc += int(pred_region == gt_region)
            pred_bbox_px = region_id_to_bbox(pred_region, rec.width, rec.height, num_rows=num_rows, num_cols=num_cols)
        ...
        if len(image_logs) < log_image_num:
            try:
                image_logs.append(
                    swanlab.Image(
                        rec.image_path,
                        caption=f"pred={pred_text} | gt_region=s{gt_region} | gt_bbox={list(rec.bbox_px)} | iou={iou:.3f}"
                    )
                )
            except Exception:
                pass
            ...
            try:
                image_logs.append(
                    swanlab.Image(
                        rec.image_path,
                        caption=(
                            f"det_pred={det_text} | gt_region=s{gt_region} | cls_pred={cls_text} | "
...
def train_stage(
    ...
    if task == "det":
        metrics, image_logs = evaluate_detector(model, processor, val_records, num_rows=num_rows, num_cols=num_cols)
    elif task == "cls":
...
def main():
    ...
    parser.add_argument("--det_output_dir", type=str,
                        default="/home/dwd/桌面/Spinal-qwen-finetune/output/qwen35_stage1_det_5x2")
    ...
    parser.add_argument("--swanlab_experiment", type=str, default="two-stage-5x2")
