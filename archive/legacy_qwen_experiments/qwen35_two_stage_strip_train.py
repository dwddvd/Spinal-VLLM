...
def point_to_strip_id(cy: float, h: int, num_strips: int = 5) -> int:
    row = min(int(cy / max(h, 1) * num_strips), num_strips - 1)
    return row + 1


def bbox_to_strip_id(bbox: Tuple[int, int, int, int], h: int, num_strips: int = 5) -> int:
    _, cy = bbox_center(bbox)
    return point_to_strip_id(cy, h, num_strips=num_strips)


def strip_id_to_bbox(strip_id: int, w: int, h: int, num_strips: int = 5) -> Tuple[int, int, int, int]:
    idx = strip_id - 1
    strip_h = h / num_strips

    x1 = 0
    x2 = w
    y1 = int(round(idx * strip_h))
    y2 = int(round((idx + 1) * strip_h))

    return clamp_bbox((x1, y1, x2, y2), w, h)


def extract_region_from_text(text: str, num_strips: int = 5) -> Optional[int]:
    m = re.search(r"region\s*[:：]\s*z(\d+)", text, flags=re.I)
    if not m:
        return None
    rid = int(m.group(1))
    if 1 <= rid <= num_strips:
...
# =====================================
# Stage-1 Strip detector prompts/answers
# =====================================

def build_det_prompt(seq: Optional[str], num_strips: int = 5) -> str:
    seq_part = f"，序列为{seq}" if seq else ""
    region_list = ",".join([f"z{i}" for i in range(1, num_strips + 1)])
    return (
        f"现在你是一个骨科专家，这是一幅脊椎的磁共振图像{seq_part}。"
        f"请观察整张图像，判断图中最主要病灶位于哪个纵向条带区域。"
        f"整张图像从上到下被划分为{num_strips}个纵向条带区域，"
        f"区域编号为：{region_list}。"
        f"请严格按照以下格式回答：region:z?"
    )


def build_det_answer(bbox_px: Tuple[int, int, int, int], h: int, num_strips: int = 5) -> str:
    region_id = bbox_to_strip_id(bbox_px, h, num_strips=num_strips)
    return f"region:z{region_id}"
...
class TwoStageDatasetBuilder:
    def __init__(self, processor, task: str, max_length: int, crop_expand_ratio: float = 0.1, num_strips: int = 5):
    ...
    def _build_messages_and_answer(self, example: Dict):
        ...
        if self.task == "det":
            prompt = build_det_prompt(seq, num_strips=self.num_strips)
            answer = build_det_answer(bbox, height, num_strips=self.num_strips)
...
def predict_region(det_model, det_processor, image_path: str, seq: Optional[str], max_new_tokens: int = 16, num_strips: int = 5):
    prompt = build_det_prompt(seq, num_strips=num_strips)
    ...
    region_id = extract_region_from_text(text, num_strips=num_strips)
    return text, region_id


def predict_label(cls_model, cls_processor, image_path: str, bbox_px: Tuple[int, int, int, int], seq: Optional[str], max_new_tokens: int = 16):
    img = Image.open(image_path).convert("RGB")
...
def evaluate_detector(model, processor, records: List[Record], log_image_num: int = 20, num_strips: int = 5):
    total = 0
    region_acc = 0
    iou03 = 0
    iou05 = 0
    ...
    pbar = tqdm(records, desc="Eval Detector", ncols=120)
    for rec in pbar:
        total += 1

        pred_text, pred_region = predict_region(
            model, processor, rec.image_path, rec.seq, max_new_tokens=16, num_strips=num_strips
        )

        gt_region = bbox_to_strip_id(rec.bbox_px, rec.height, num_strips=num_strips)

        iou = 0.0
        if pred_region is not None:
            region_acc += int(pred_region == gt_region)
            pred_bbox_px = strip_id_to_bbox(pred_region, rec.width, rec.height, num_strips=num_strips)
            iou = compute_iou(pred_bbox_px, rec.bbox_px)
            iou03 += int(iou >= 0.3)
            iou05 += int(iou >= 0.5)

        pbar.set_postfix(
            region_acc=f"{region_acc/max(total,1):.4f}",
        ...
        if len(image_logs) < log_image_num:
            try:
                image_logs.append(
                    swanlab.Image(
                        rec.image_path,
                        caption=f"pred={pred_text} | gt_region=z{gt_region} | gt_bbox={list(rec.bbox_px)} | iou={iou:.3f}"
                    )
                )
            except Exception:
                pass
    ...
    metrics = {
        "det_region_acc": region_acc / max(total, 1),
...
def evaluate_pipeline(det_model, det_processor, cls_model, cls_processor, records: List[Record], log_image_num: int = 20, num_strips: int = 5):
    total = 0
    region_acc = 0
    det03 = 0
    det05 = 0
    ...
    for rec in pbar:
        ...
        det_text, pred_region = predict_region(
            det_model, det_processor, rec.image_path, rec.seq, max_new_tokens=16, num_strips=num_strips
        )
            ...
            try:
                image_logs.append(
                    swanlab.Image(
                        rec.image_path,
                        caption=(
                            f"det_pred={det_text} | gt_region=z{gt_region} | cls_pred={cls_text} | "
                            f"gt_bbox={list(rec.bbox_px)} | gt_label={rec.label} | iou={iou:.3f}"
                        )
                    )
                )
            except Exception:
    ...
    metrics = {
        "pipeline_region_acc": region_acc / max(total, 1),
...
def train_stage(
    ...
    if task == "det":
        metrics, image_logs = evaluate_detector(model, processor, val_records, num_strips=num_strips)
    elif task == "cls":
...
def main():
    ...
    parser.add_argument("--det_output_dir", type=str,
                        default="/home/dwd/桌面/Spinal-qwen-finetune/output/qwen35_stage1_det_strip")
    ...
    parser.add_argument("--num_strips", type=int, default=5)

    parser.add_argument("--swanlab_project", type=str, default="Qwen3.5-VL-LoRA")
    parser.add_argument("--swanlab_experiment", type=str, default="two-stage-strip")
    ...
    if args.mode == "train_det":
        ...
        metrics, image_logs = evaluate_pipeline(
            det_model, det_processor, cls_model, cls_processor, val_records, num_strips=args.num_strips
        )
        print(json.dumps(metrics, ensure_ascii=False, indent=2))
        swanlab.log(metrics)
        if image_logs:
